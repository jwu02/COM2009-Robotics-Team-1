...
class ExploreServer():
    ...
    def action_server_launcher(self, goal: Task5ExplorationGoal):
        ...
        self.lim_ang_vel = (self.lin_vel / AVOIDANCE_DISTANCE) * 1.3
        ...
        def generate_step_size() -> float:
            ...
            if x==0: return 2 + random()*2
            elif x==1: return 4 + random()*2
        ...
        while rospy.get_rostime() < (self.request_time + self.assignment_time_limit + rospy.Duration(180)):
            ...
            if self.robot_scan.front_min_distance <= AVOIDANCE_DISTANCE*1.2:
                ...
                if step_distance_travelled < step_size:
                    # # check if rearend stuck and robot hasn't moved
                    # if (self.robot_odom.total_distance == self.previous_distance_travelled) and \
                    #         self.robot_scan.rear_min_distance <= AVOIDANCE_DISTANCE*0.7:
                    #     self.robot_controller.set_move_cmd(0.05, 0)
                    # else:
                    ...
                    print(f"{step_size=}[m]. {step_distance_travelled=}[m]. {self.robot_scan.front_min_distance=}[m]. ")
                else:
